...
def text_in_rect(canvas, text, font, rect, line_spacing=1.1):
	...
	# タイトルに日本語が含まれる場合はMeCabを使う。そうでない場合は英語を想定し半角スペースで切る
        # ord関数でtextに含まれる文字のUnicodeコードポイントを参照している
        # 別途日本語の処理がしやすいようにja_flagを立てる
        if any([c for c in text if 0x3040 <= ord(c) <= 0x30FF or 0x4E00 <= ord(c) <= 0x9FFF]):
             res = tagger.parse(text)
             words = [str(word) for word in res.split()]
             words.reverse()
             ja_flag = True
        else:
             words = text.split(" ")
...
class DisplayPIL(Display):
	...
	#from fonts.ttf import KosugiMaru as UserFont
